...
def crop_imges(img, seg, shape):
    ...
    if x==0 and y==0 and w==255 and h==255: return resize(img, shape)
    cX = int(x + w/2)
    cY = int(y + h/2)
    tform = AffineTransform(translation=(-cY + int(seg.shape[1]/2), -cX + int(seg.shape[0]/2)))
    img = warp(img, tform.inverse)
    cX = int(img.shape[0]/2)
    cY = int(img.shape[1]/2)
    
    if seg[seg==1].size/seg.size < 0.02: 
        
        x = cX - int(0.15*seg.shape[0]/2)
        w = int(0.15*seg.shape[0])
        
        y = cY - int(0.4*img.shape[0]/2)
        h = int(0.15*seg.shape[1]) 
        
    if w > h :
        x = cX - int(w/2)
        y = cY - int(w/2)
        new_img = img[x : x + w, y : y + w]
    else:
        x = cX - int(h/2)
        y = cY - int(h/2)
        new_img = img[x : x + h, y : y + h]
    
    return resize(new_img, shape)
